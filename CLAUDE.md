...
## Pages and rendering
- `app/page.tsx` — `export const dynamic = 'force-dynamic'` (reads Blobs at request time)
- `app/projects/[slug]/page.tsx` — `export const dynamic = 'force-dynamic'`, no `generateStaticParams` (dynamic because project list can change from admin)
- All public-facing components (`hero`, `projects`, `about`, `contact`) are **async server components**
